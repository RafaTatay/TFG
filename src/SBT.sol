...
contract SBT {
    ...
    function setProfile(address _soul, Soul memory _soulData) external {
        require(keccak256(bytes(souls[_soul].id)) != zeroHash, "Cannot create a profile for a soul that has not been minted");
    ...
    function hasProfile(address _profiler, address _soul) external view returns (bool) {
        if (keccak256(bytes(soulProfiles[_profiler][_soul].id)) == zeroHash) {
            return false;
        } else {
            return true;
        }
